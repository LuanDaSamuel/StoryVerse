...
import { BackIcon, BookOpenIcon, ChevronDownIcon, ChevronLeftIcon, ChevronRightIcon, TextIcon, SearchIcon, BoldIcon, ItalicIcon, UndoIcon, RedoIcon } from '../components/Icons';
...
const ChapterEditorPage: React.FC = () => {
    ...
    const handleFindAndReplace = (replaceAll: boolean = false) => {
            ...
            `}>
                <div className={`w-80 h-full ${themeClasses.bgSecondary} ${themeClasses.accentText} text-sm font-sans border-l ${themeClasses.border} flex flex-col`}>
                    ...
                    <div className="flex-1 overflow-y-auto">
                        ...
                        <hr className={`border-t ${themeClasses.border} w-full`}/>
                        <Accordion title="Find & Replace" icon={<SearchIcon className="w-5 h-5" />} isOpen={openAccordion === 'find'} onToggle={() => handleAccordionToggle('find')} fullWidthBorder>
                            <div className="space-y-3">
                                <input type="text" placeholder="Find..." value={findText} onChange={(e) => setFindText(e.target.value)} className={`w-full px-3 py-2 rounded-md ${themeClasses.input}`} />
                                <input type="text" placeholder="Replace with..." value={replaceText} onChange={(e) => setReplaceText(e.target.value)} className={`w-full px-3 py-2 rounded-md ${themeClasses.input}`} />
                                <div className="flex space-x-2">
                                    <button onClick={() => handleFindAndReplace(false)} className={`w-full py-2 rounded-md ${themeClasses.bgTertiary} hover:opacity-80`}>Replace</button>
                                    <button onClick={() => handleFindAndReplace(true)} className={`w-full py-2 rounded-md ${themeClasses.bgTertiary} hover:opacity-80`}>Replace All</button>
                                </div>
                            </div>
                        </Accordion>
                    </div>
