            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {novels.map((novel) => (
                    <ReactRouterDOM.Link
                        to={`/novel/${novel.id}`}
                        key={novel.id}
                        className={`group block rounded-lg overflow-hidden transition-all duration-300 ease-in-out transform hover:-translate-y-1 hover:shadow-2xl ${themeClasses.bgSecondary}`}
                    >
                        {/* Cover Image */}
                        ...
                        <div className="p-4">
                            ...
                            <p className={`text-sm ${themeClasses.textSecondary} overflow-hidden`} style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
                                {enhancePlainText(novel.description) || <span className="italic opacity-70">No description available.</span>}
                            </p>
                        </div>
                    </ReactRouterDOM.Link>
                ))}
